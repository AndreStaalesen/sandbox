...
//array funskjon for p pushe til html
let articleContent = [];
const ressurs = () =>{
    let navItems = "";

    resources.map((res, index) => {
        navItems += `<button onClick="" aria-role="navigation" id="nav-${index}" class="btn">${res.category}</button>`
        articleContent.push(
    ...
    document.querySelector("#printarea").innerHTML = articleContent[0];
...
function toggle(id){
    ...
    document.querySelector("#printarea").innerHTML = articleContent(id)
    buttons.forEach(e => e.classList.remove("active"))

    if(id == articleContent.indexOf(articleContent[id])) {
